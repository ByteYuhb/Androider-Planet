...
static const char* invokeClass = "com/android/hellojni/HelloCallBack";
...
void callJavaField(JNIEnv* env,jobject obj,jstring className,jstring fieldName){
    ...
    //此处如果传入一个找不到的字段会报错，如果不做异常处理，应用直接会崩溃，为了更好的知晓问题所在，需要使用jni异常机制
    /*
    ...
    if(hasException == JNI_TRUE){
        ...
        /*char* msgHeader = "找不到该字段";
        strcat(msgHeader,_fieldName);*/
